...
<section class="mb-32 mt-10">
    <h3 class="text-3xl font-bold mb-3 text-center">Edit Disease Post</h3>
    <section class="bg-white dark:bg-gray-900">
        <div class="py-8 px-4 mx-auto max-w-2xl lg:py-8">
            <form action="{{route('disease.update', $disease->id)}}" method="POST" enctype="multipart/form-data" class="bg-slate-100 rounded-lg p-8">
                @csrf
                @extends('layouts.message')
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    <div class="sm:col-span-2">
                        <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Disease Name</label>
                        <input type="text" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500 @error('name') border-red-600 @enderror" placeholder="Type New Disease name" value="{{old('name', $disease->name)}}">
                        @error('name')
